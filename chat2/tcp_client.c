...
int buffer_size = 1024;  //размер сообщения
...
int main(int argc, char * argv[])
{
    ...
    /* настроить сокет для передачи TCP  */
    socket_num = tcp_send_setup(argv[2], argv[3]);

	//	Установите сокеты для select () на: stdin и server_socket
    sockets[0] = 2;
...
{
    int socket_num;
    struct sockaddr_in remote;       // адрес сокета для удаленной стороны
    struct hostent *hp;              // адрес удаленного хоста
...
void tcp_receive() {
	...
	//Теперь получить данные на server_socket
...
void serverMessageError(int socket, char *buf, int message_len) {
	...
	printf("~$ Client with login %s does not exist\n", handle);
}
